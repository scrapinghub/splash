...
def parse_opts(jupyter=False, argv=None):
    ...
    op.add_option("--maxrss_timeout", help="set custom interval for checking rss (in seconds) (default: %default)", default=60)
...
def monitor_maxrss(maxrss, maxrss_timeout):
    ...
    if maxrss:
        ...
        t.start(maxrss_timeout, now=False)
...
def main(jupyter=False, argv=None, server_factory=splash_server):
    ...
    with xvfb.autostart(opts.disable_xvfb, opts.xvfb_screen_size) as x:
        ...
        log_splash_version()
        bump_nofile_limit()
        monitor_maxrss(opts.maxrss, opts.maxrss_timeout)
